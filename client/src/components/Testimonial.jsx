                ...
                {dummyTestimonialData.map((testimonial, index) => (
                    <div key={index} className='p-8 m-4 max-w-xs rounded-lg bg-[#FDFDFE] shadow-lg border border-gray-100 hover:-translate-y-1 transition duration-300 cursor-pointer'>
                        <div className="flex items-center gap-1">
                           {Array(5).fill(0).map((_, img)=> (<img key={img} src={img < testimonial.rating ? assets.star_icon : assets.star_dull_icon} className='w-4 h-4 object-contain inline-block' alt=" rating star" />))}
                        </div>
